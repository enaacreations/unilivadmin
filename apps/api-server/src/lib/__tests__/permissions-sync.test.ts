...
describe("RBAC matrix — menu planning personas (M15)", () => {
  // Menu Planning seeds its property picker from GET /properties, which carries
  // `authorizeAny(["PROPERTIES", "MENU_PLANNING"], "view")` — so the roles that
  // own the page reach that ONE list handler through their MENU_PLANNING grant.
  // They must NOT hold the PROPERTIES cell: it also opens
  // /properties/assignable-unit-leads, which has no scoping and returns the
  // name, email and role of every unit lead in the org. And generate-indent
  // mints a procurement document, so the INDENTS grant has to be explicit
  // rather than riding in on MENU_PLANNING:edit.
  const OWNERS: UserRole[] = ["KITCHEN_MANAGER", "FNB_MANAGER"];

  it("lets the menu-planning owners reach the property list through MENU_PLANNING", () => {
    for (const role of OWNERS) {
      expect(granted(BACKEND, role, "MENU_PLANNING", "edit")).toBe(true);
      expect(granted(BACKEND, role, "MENU_PLANNING", "view")).toBe(true);
    }
  });

  it("does NOT give the menu-planning owners the PROPERTIES module", () => {
    // Widening this back re-opens the unscoped staff roster. If GET /properties
    // 403s for these roles, fix the route's authorizeAny — not this cell.
    for (const role of OWNERS) {
      expect(granted(BACKEND, role, "PROPERTIES", "view")).toBe(false);
    }
  });

  it("gives the menu-planning owners create-only access to indents", () => {
    for (const role of OWNERS) {
      expect(granted(BACKEND, role, "INDENTS", "create")).toBe(true);
      // The indent register is org-wide and unscoped — no read, no approve.
