...
export const ROLE_PERMISSIONS: Record<UserRole, Partial<Record<Module, Partial<Record<Permission, boolean>>>>> = {
  ...
  FNB_MANAGER: {
    ...
    // the portion; the dish drawer says so rather than blocking the save. The
    // pages derive this from `myKitchenIds` (orgWideConfig), not from the role.
    // Keep this in sync with the same block in apps/api-server/src/lib/permissions.ts.
    FOOD_SETTINGS: VE,
