...
coursesRouter.get("/stats", authenticate, async (_req, res) => {
  try {
...
coursesRouter.get("/:id", authenticate, async (req, res) => {
  try {
...
coursesRouter.delete("/:id", authenticate, async (req, res) => {
  try {
...
coursesRouter.get("/:id/enrollments", authenticate, async (req, res) => {
  try {
...
// bulk enroll
coursesRouter.post("/:id/enroll", authenticate, async (req, res) => {
  try {
...
// send reminders to incomplete enrollments (stub)
coursesRouter.post("/:id/remind", authenticate, async (req, res) => {
  try {
    const courseId = req.params["id"]!;
    ...
    req.log.info({ courseId, count: incomplete.length }, "[email/SMS stub] would send reminders");
    res.json({ success: true, data: { sent: incomplete.length } });
  } catch (err) { req.log.error(err); res.status(500).json({ success: false, error: "Internal server error" }); }
});
...
enrollmentsRouter.post("/:id/progress", authenticate, async (req, res) => {
  try {
...
enrollmentsRouter.post("/:id/quiz", authenticate, async (req, res) => {
  try {
